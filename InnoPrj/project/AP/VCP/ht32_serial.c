...
#endif
//static void VCP_Timeout_Restart(void);
//static void VCP_Timeout_Stop(void);
...
vu8 VCP_RX_finish = FALSE;
u8 VCP_RX_Buffer[MAX_CMD_LEN + BUFFER_ADDITIONAL_SIZE];
Buffer_TypeDef VCP_RX_Index;
...
void USBDClass_Init(USBDCore_Class_TypeDef *pClass)
{
  ...
  Buffer_Init(&VCP_RX_Index, VCP_RX_Buffer, MAX_CMD_LEN+BUFFER_ADDITIONAL_SIZE);
  return;
}
...
void SERIAL_USBDInit(void)
{
  ...
  CKCU_PeripClockConfig(CKCUClock, ENABLE);
	...
	NVIC_SetPriority(QSPI_IRQn, 0);
	NVIC_SetPriority(MIDI_IRQn, 1);
	NVIC_SetPriority(BFTM0_IRQn, 2);
	NVIC_SetPriority(SysTick_IRQn, 2);
	NVIC_SetPriority(USB_IRQn, 2);
	NVIC_SetPriority(UART0_IRQn, 3);
	NVIC_SetPriority(USART0_IRQn, 3);
	NVIC_SetPriority(BFTM1_IRQn, 3);
	NVIC_SetPriority(SCTM0_IRQn, 3);
	NVIC_SetPriority(SCTM1_IRQn, 3);
...
void SERIAL_USBDClass_RXHandler(USBD_EPTn_Enum EPTn)
{
  ...
  Buffer_Write(&VCP_RX_Index, uTempBuffer, uLen);
  if(uLen < 64)
    VCP_RX_finish = TRUE;
//  if(uLen < 64)
//  {
...
void printhex(u8 *buffer, u8 len)
{
  for(u8 i = 0;i < len;i++)
  {
