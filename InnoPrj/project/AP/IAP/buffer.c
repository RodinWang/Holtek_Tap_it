...
#define BUFF_FERR_LENGTH(Read, Write, Size)       ((Write >= Read) ? (Size - Write + Read) : (Read - Write))
/* Global variables ----------------------------------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------------------------------------*/
/* Global functions ----------------------------------------------------------------------------------------*/
...
/*********************************************************************************************************//**
  * @brief  Read byte from buffer depend on specific index.
  * @param  pBuffer: Pointer of Buffer_TypeDef
  * @param  puData: Pointer of returned data
  * @param  uIndex: Specific byte to be read, form zero start
  * @retval uResult: TRUE for read success, FALSE for no DATA
  * @note   The read point will not move
  ***********************************************************************************************************/
u32 Buffer_ReadByteExt(Buffer_TypeDef *pBuffer, u8 *puData, u32 uIndex)
{
  u32 uResult = FALSE;
  u32 index;
  if(uIndex < Get_Valid_Lenth(pBuffer))
  {
    if(pBuffer->uRead + uIndex < pBuffer->uSize)
    {
      index = pBuffer->uRead + uIndex;
    }
    else
    {
      index = (pBuffer->uRead + uIndex) % pBuffer->uSize;
    }
    *puData = pBuffer->puMemory[index];
    uResult = TRUE;
  }
...
/*********************************************************************************************************//**
  * @brief  Extend read buffer
  * @param  pBuffer: Pointer of Buffer_TypeDef
  * @param  puData: Pointer of returned data
  * @param  uIndex: Specific position to be read, form zero start
  * @param  uLength: Read length
  * @retval uCount: Data length of read operation
  * @note   The read point will not move
  ***********************************************************************************************************/
u32 Buffer_ReadExt(Buffer_TypeDef *pBuffer, u8 *puData, u32 uIndex, u32 uLength)
{
  u32 uCount;
  u32 uResult;

  for (uCount = 0; uCount < uLength; uCount++,uIndex++)
  {
    uResult = Buffer_ReadByteExt(pBuffer, (u8 *)(puData + uCount), uIndex);
    if (!uResult)
    {
      break;
    }
...
/*********************************************************************************************************//**
  * @brief  Extend Write Buffer,when free length is not enough,the function will return zero
  * @param  pBuffer: Pointer of Buffer_TypeDef
  * @param  puData: Pointer of dada buffer for write
  * @param  uLength: Write length
  * @retval uCount: Data length of write operation
  ***********************************************************************************************************/
u32 Buffer_WriteExt(Buffer_TypeDef *pBuffer, u8 *puData, u32 uLength)
{
	u32 freeLength = BUFF_FERR_LENGTH(pBuffer->uRead, pBuffer->uWrite, pBuffer->uSize) - 1;
	if(freeLength < uLength)return 0;
	
	return Buffer_Write(pBuffer, puData, uLength);
}
...
{
  return pBuffer->uSize - (BUFF_FERR_LENGTH(pBuffer->uRead, pBuffer->uWrite, pBuffer->uSize));
}
