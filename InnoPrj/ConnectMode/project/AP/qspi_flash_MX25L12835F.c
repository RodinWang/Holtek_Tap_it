...
void QSPI_FLASH_Init(void)
{
  ...
  //PC10--SEL,PC11--SCK
  //PC12--SIO0,PC13--SIO1
  GPIO_PullResistorConfig(HT_GPIOA, GPIO_PIN_8 | GPIO_PIN_10, GPIO_PR_DOWN);
  GPIO_PullResistorConfig(HT_GPIOC, GPIO_PIN_12 | GPIO_PIN_13, GPIO_PR_DOWN);
...
/*********************************************************************************************************//**
  ...
  * @retval None
  ***********************************************************************************************************/
void QSPI_FLASH_BufferWrite(u8* WriteBuffer, u32 WriteAddr, u32 NumOfWriteByte)
{
  ...
  NumOfPage =  NumOfWriteByte / QSPI_FLASH_PageSize;
  NumOfSingle = NumOfWriteByte % QSPI_FLASH_PageSize;

  if (Addr == 0)
  {
    if (NumOfPage == 0)
    {
      QSPI_FLASH_BYTE_QPP(WriteBuffer, WriteAddr, NumOfWriteByte);
    }
    else
    {
      while (NumOfPage--)
      {
        QSPI_FLASH_BYTE_QPP(WriteBuffer, WriteAddr, QSPI_FLASH_PageSize);
        WriteAddr +=  QSPI_FLASH_PageSize;
        WriteBuffer += QSPI_FLASH_PageSize;
      }
      if (NumOfSingle)
      {
        QSPI_FLASH_BYTE_QPP(WriteBuffer, WriteAddr, NumOfSingle);
      }
    }
  }
  else
  {
    if (NumOfPage == 0)
    {
      if (NumOfSingle > Count)
      {
        ...
        QSPI_FLASH_BYTE_QPP(WriteBuffer, WriteAddr, Count);
        WriteAddr +=  Count;
        WriteBuffer += Count;

        QSPI_FLASH_BYTE_QPP(WriteBuffer, WriteAddr, tmp);
      }
      else
      {
        QSPI_FLASH_BYTE_QPP(WriteBuffer, WriteAddr, NumOfWriteByte);
...
//    time_out--;
  ...
  } while((FLASH_Status & WIP_Flag) == SET/* && time_out*/); /* Write in progress */
